...
const HistoryScreen: React.FC<HistoryScreenProps> = ({ onBack, onNavigate }) => {
  ...
  const { contracts, removeContract, clearContracts } = useContract();
  ...
  // Comprehensive fallback loading function
  const loadLocalDataFallback = async (): Promise<ExtendedSessionDetails[]> => {
    console.log('🔄 Loading local data fallback...');
    const fallbackSessions: SessionDetailsApiResponse[] = [];

    try {
      // Method 1: Try getLocalSessions (API function)
      console.log('📚 Trying getLocalSessions...');
      const localSessions = await getLocalSessions();
      if (localSessions && localSessions.length > 0) {
        console.log('✅ Found sessions via getLocalSessions:', localSessions.length);
        fallbackSessions.push(...localSessions);
      }
    } catch (error) {
      console.warn('⚠️ getLocalSessions failed:', error);
    }

    try {
      // Method 2: Try getAllStoredSessions (utility function)
      console.log('📚 Trying getAllStoredSessions...');
      const storedSessions = await getAllStoredSessions();
      if (storedSessions && storedSessions.length > 0) {
        console.log('✅ Found sessions via getAllStoredSessions:', storedSessions.length);
        // Merge without duplicates
        storedSessions.forEach(session => {
          if (!fallbackSessions.find(s => s.session_id === session.session_id)) {
            fallbackSessions.push(session);
          }
        });
      }
    } catch (error) {
      console.warn('⚠️ getAllStoredSessions failed:', error);
    }

    try {
      // Method 3: Try getOfflineAnalyses and convert to sessions
      console.log('📚 Trying getOfflineAnalyses...');
      const offlineAnalyses = await getOfflineAnalyses();
      if (offlineAnalyses && offlineAnalyses.length > 0) {
        console.log('✅ Found offline analyses:', offlineAnalyses.length);
        offlineAnalyses.forEach(analysis => {
          if (analysis.fullSessionData && !fallbackSessions.find(s => s.session_id === analysis.sessionId)) {
            fallbackSessions.push(analysis.fullSessionData);
          }
        });
      }
    } catch (error) {
      console.warn('⚠️ getOfflineAnalyses failed:', error);
    }

    try {
      // Method 4: Direct storage access for shariaa_sessions
      console.log('📚 Trying direct storage access...');
      const directData = await storage.getItemAsync(storageKeys.SHARIAA_SESSIONS);
      if (directData) {
        const parsedData = JSON.parse(directData);
        if (Array.isArray(parsedData)) {
          console.log('✅ Found direct storage sessions:', parsedData.length);
          parsedData.forEach(session => {
            if (session.session_id && !fallbackSessions.find(s => s.session_id === session.session_id)) {
              fallbackSessions.push(session);
            }
          });
        }
      ...
      console.warn('⚠️ Direct storage access failed:', error);
    }

    console.log(`🔄 Total fallback sessions found: ${fallbackSessions.length}`);
    return await enrichSessionData(fallbackSessions);
  };

  const loadHistory = useCallback(async (showRefresh = false) => {
    console.log('📚 Starting loadHistory, showRefresh:', showRefresh);

    if (showRefresh) setRefreshing(true);
    else setLoading(true);

    try {
      // Always start with comprehensive fallback for immediate display
      console.log('📚 Loading local data first...');
      const localSessions = await loadLocalDataFallback();

      if (localSessions.length > 0) {
        console.log('✅ Setting local sessions:', localSessions.length);
      ...
      try {
        ...
        if (remoteHistory && remoteHistory.length > 0) {
          console.log('✅ Remote data loaded, enriching:', remoteHistory.length);
          const enrichedRemoteData = await enrichSessionData(remoteHistory);

          // Merge remote with local, prioritizing remote data
          const mergedSessions = [...enrichedRemoteData];
          localSessions.forEach(localSession => {
            if (!enrichedRemoteData.find(s => s.session_id === localSession.session_id)) {
              mergedSessions.push(localSession);
            }
          });

          setSessions(mergedSessions);
          console.log('✅ Set merged sessions:', mergedSessions.length);
        } else {
          console.log('⚠️ No remote data, keeping local sessions');
        }
      } catch (remoteError) {
        const errorMessage = remoteError instanceof Error ? remoteError.message : String(remoteError);
        console.log('⚠️ Remote history not available, using local data only:', errorMessage);
      ...
      try {
        const emergencyFallback = await loadLocalDataFallback();
        setSessions(emergencyFallback);
        console.log('🚨 Emergency fallback loaded:', emergencyFallback.length);
  ...
  useEffect(() => {
    console.log('📊 Sessions state changed:', {
      ...
      sessionIds: sessions.map(s => s.session_id).slice(0, 3), // First 3 IDs for debugging
      loading,
      refreshing
    });
  ...
  const manualRefresh = async () => {
    ...
    setLoading(true);
    try {
      const fallbackData = await loadLocalDataFallback();
      setSessions(fallbackData);
      console.log('✅ Manual refresh completed:', fallbackData.length);

      if (fallbackData.length === 0) {
        console.log('⚠️ Still no data found, checking storage keys...');
        ...
        try {
          ...
          for (const key of keys) {
            ...
            console.log(`🔍 Debug storage[${key}]:`, value ? 'HAS_DATA' : 'NULL');
          }
        } catch (debugError) {
          console.error('🔍 Debug storage check failed:', debugError);
        }
      }
    } catch (error) {
