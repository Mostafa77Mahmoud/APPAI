...
import { validateAllTranslations, createI18nDevTools } from './utils/i18nValidator';
...
const MobileApp: React.FC = () => {
  ...
  useEffect(() => {
    const initializeApp = async (): Promise<(() => void) | undefined> => {
      try {
        ...
        console.log('🔑 App initialized with device ID:', deviceId.substring(0, 8) + '...');
    ...
    const checkStoredDocument = async () => {
      try {
        if (Platform.OS === 'web') {
          ...
          const storedDoc = await AsyncStorage.default.getItem('temp_camera_document');
          if (storedDoc) {
            const document = JSON.parse(storedDoc);
            setCameraGeneratedFile(document);
  ...
  const renderCurrentScreen = () => {
    ...
    switch (currentScreen) {
      ...
      case 'history':
        return <HistoryScreen onNavigate={handleNavigate} onBack={() => handleNavigate('home')} />;
      case 'profile':
...
export type ScreenType = 'home' | 'upload' | 'camera' | 'history' | 'profile' | 'results';